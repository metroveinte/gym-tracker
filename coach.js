...
- set_scheme: elige el esquema adecuado para cada ejercicio según su posición en la sesión y el objetivo:
    ...
    "calentamiento_trabajo" → EXACTAMENTE 1 set ligero de activación (el primero) + el resto de sets al peso objetivo
- CONSISTENCIA OBLIGATORIA entre set_scheme, set_scheme_note y session_weights_week1: los tres deben describir
  literalmente lo mismo para cada ejercicio. Si set_scheme_note menciona una serie de activación, set_scheme
  DEBE ser "calentamiento_trabajo" Y session_weights_week1[0] DEBE llevar el peso reducido (65-70%) — nunca
  describas una activación que luego no aparece en los pesos reales. Si el ejercicio usa "rectas", set_scheme_note
  NO debe mencionar ninguna activación ni "últimos N sets al objetivo" — todos los sets son iguales, sin excepción.
...
- peso de activación: para ejercicios con esquema "calentamiento_trabajo", la serie de activación (SOLO la primera, nunca más de una) se carga siempre al 65-70% del peso de trabajo actual. Este porcentaje es fijo y escala automáticamente: si el peso de trabajo sube de 60 kg a 65 kg, la activación pasa de ~40 kg a ~43 kg sin necesidad de recalcular. Indica este porcentaje UNA sola vez, en el set_scheme_note.
- session_weights_week1: array con el peso exacto de cada set en la semana 1 (longitud = sets). Para "rectas" TODOS los elementos deben ser idénticos, sin ninguna excepción. Para pirámide, mostrar la progresión real. Para "calentamiento_trabajo": el PRIMER elemento (y solo el primero) es el peso de activación (~65-70% del peso de trabajo, redondeado a 2.5kg); todos los demás elementos son el peso de trabajo, idénticos entre sí. Usa siempre la unidad (kg o "PC").
